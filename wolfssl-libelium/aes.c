...
/* AES-CBC */
...
#else
    /* 
     * LH: Compiler optimisation to avoid error:
     *
     *       "unable to find a register to spill in class ‘POINTER_REGS’"
     * 
     * 
     */
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz) __attribute__((__optimize__("O2")));
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
     ...
     * 
     */
    int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz) __attribute__((__optimize__("O2")));
    int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz) 
    {
        word32 blocks = sz / AES_BLOCK_SIZE;
